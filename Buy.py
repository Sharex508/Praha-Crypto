...
def get_coin_limits():
    """Fetch coin limits from the database."""
    connection, cursor = get_db_connection()
    try:
        sql_limits = "SELECT margin3count, margin5count, margin10count, margin20count FROM Coinnumber"
...
def task(db_resp, api_resp, coin_limits, trading_sums, data):
...
def get_diff_of_db_api_values(api_resp):
    ...
    trading_sums = get_trading_sums()
    
    if not db_resp or not coin_limits or not trading_sums:
        logging.error("Error: Failed to retrieve necessary data from the database.")
        return None, None, None

    return db_resp, coin_limits, trading_sums
...
def show():
    ...
    while True:
        try:
            ...
            db_resp, coin_limits, trading_sums = get_diff_of_db_api_values(api_resp)
            if not db_resp:
                time.sleep(60)
                continue
            ...
            with ThreadPoolExecutor(max_workers=4) as executor:
                for chunk in chunks:
                    executor.submit(task, db_resp, api_resp, coin_limits, trading_sums, chunk)
